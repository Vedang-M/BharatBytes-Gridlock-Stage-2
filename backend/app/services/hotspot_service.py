...
class HotspotService:
    ...
    def __init__(self):
        ...
        # Initialize with empty clusters – will be loaded in background
        self.df_clust = None
        self.clusters = None
        ...
        thread = threading.Thread(target=self._load_clusters_async, daemon=True)
        thread.start()
    
    def _load_clusters_async(self):
        """Load or compute clusters in background thread."""
        cache_path = os.path.join(os.path.dirname(__file__), "..", "..", "clusters_cache.pkl")
        ...
        if os.path.exists(cache_path):
            try:
                with open(cache_path, 'rb') as f:
                    ...
                    with self._lock:
                        self.df_clust = cached['df_clust']
                        self.clusters = cached['clusters']
                        self._clustering_complete = True
                print(f"[HotspotService] Loaded cached clusters from {cache_path}")
                return
            except Exception as e:
                print(f"[HotspotService] Cache load failed ({e}), recomputing...")
        
        print(f"[HotspotService] Computing clusters in background (may take 30-60s)...")
        try:
            df_clust, clusters = self._cluster(self.df)
            with self._lock:
                self.df_clust = df_clust
            ...
            try:
                with open(cache_path, 'wb') as f:
                    pickle.dump({'df_clust': df_clust, 'clusters': clusters}, f)
                print(f"[HotspotService] Cached clusters to {cache_path}")
            except Exception as e:
                print(f"[HotspotService] Cache save failed ({e}), continuing without cache")
        except Exception as e:
            print(f"[HotspotService] Clustering failed: {e}")
            with self._lock:
                self._clustering_complete = True
    
    def wait_for_clusters(self, timeout=120):
        """Wait for clustering to complete. Returns True if ready, False if timeout."""
        start = time.time()
        while not self._clustering_complete and (time.time() - start) < timeout:
            time.sleep(0.1)
        return self._clustering_complete and self.df_clust is not None
